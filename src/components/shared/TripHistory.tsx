...
const RouteDisplay = ({ pickup, destinations, destination }: { pickup: string, destinations?: string[], destination: string }) => {
    ...
    const allPoints = [pickup, ...destinations, destination]; // Include final destination
...
export function TripHistory({ user, onNavigate, onLogout }: TripHistoryProps) {
    ...
    // Function to reload all trips (used after cancellation)
    const refreshTrips = () => {
        setLoading(true);
    ...
    useEffect(() => {
        refreshTrips();
    }, [user.id, user.role]); // Depend on user ID/role for initial load

    const filteredTrips = trips.filter((trip) => {
        const vehicle = trip.vehicleNumber || trip.vehicle || '';
        const epf = trip.epfNumber || trip.epf || '';
        const tripIdentifier = trip.serialNumber || trip.id;
        
        // NEW FILTER: Trip ID
        if (filters.tripId && !tripIdentifier.toLowerCase().includes(filters.tripId.toLowerCase())) return false;
    ...
    // --- ADMIN ACTION: CANCEL TRIP ---
    const handleCancelTrip = async (trip: any) => {
        if (user.role !== 'admin') return;

        if (!confirm(`Are you sure you want to CANCEL Trip #${trip.serialNumber || trip.id}? This action is irreversible.`)) {
            return;
        }
        ...
        try {
            ...
            await updateDoc(tripRef, {
                ...
                cancellationReason: 'Admin Cancellation from History Panel' 
            });

            // 2. Unassign vehicle (if one was assigned)
            if (trip.vehicleId && trip.driverId) {
                
                await logAction(user.email, 'TRIP_CANCELLED', 
                    `Trip ${trip.serialNumber || trip.id} cancelled. Vehicle: ${trip.vehicleNumber || 'N/A'}, Driver: ${trip.driverName || 'N/A'}`, 
                    { tripId: trip.id, driverId: trip.driverId, vehicleId: trip.vehicleId }
                );
            } else {
                 await logAction(user.email, 'TRIP_CANCELLED', 
                    `Trip ${trip.serialNumber || trip.id} cancelled (Pre-assignment).`, 
                    { tripId: trip.id }
                );
            }
            
            alert(`Trip #${trip.serialNumber || trip.id} has been successfully CANCELLED.`);
            
            // 3. Refresh data and clear selected trip
            setSelectedTrip(null);
            refreshTrips();

        } catch (error) {
            console.error("Error cancelling trip:", error);
            alert("Failed to cancel the trip. Check console.");
        }
    };
    ...
    const handleDownloadPDF = async (trip: any) => {
        try {
            ...
            if(trip.destinations && trip.destinations.length > 0) {
                // Inserting stops after Pickup row (row index 2 + 1)
                trip.destinations.forEach((stop: string, i: number) => {
                    rows.splice(3 + i, 0, [`Stop ${i+1}`, stop]);
                });
            }
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
                ...
                <Card className="p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        {/* NEW FILTER: Trip ID */}
                        <div>
                            <label className="block text-sm text-gray-700 mb-2">Trip ID</label>
                            <div className="relative">
                                <input type="text" value={filters.tripId} onChange={(e) => setFilters({ ...filters, tripId: e.target.value })} placeholder="TRIP-001 or ID" className="w-full px-4 py-3 pl-11 border border-gray-300 rounded-xl" />
                    ...
                    {(filters.tripId || filters.vehicleNumber || filters.epfNumber || filters.dateRange) && (
                        <button onClick={() => setFilters({ tripId: '', vehicleNumber: '', epfNumber: '', dateRange: '' })} className="mt-4 px-4 py-2 text-sm text-gray-700 border border-gray-300 rounded-xl hover:bg-gray-50">
                            Clear Filters
                        </button>
                    )}
                </Card>
